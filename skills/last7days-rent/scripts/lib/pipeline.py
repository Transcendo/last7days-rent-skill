...
def run_search(
    ...
    if fixture:
        ...
        acquisition = run_acquisition(
            plan,
            profile,
            search_provider=provider_search,
    ...
    markdown = render_markdown_report(profile, ranked, evidences, warnings, live=not fixture, source_fetches=source_fetches, acquisition=acquisition)
    evidence = render_evidence_package(profile, ranked, evidences, warnings, live=not fixture, source_fetches=source_fetches, acquisition=acquisition)
    markdown_path, evidence_path = write_outputs(reports_dir, basename, markdown, evidence)
    chat_summary = render_chat_shortlist(profile, ranked, acquisition=acquisition)
    return SearchResult(chat_summary, markdown_path, evidence_path, markdown, evidence, source_fetches, acquisition)
